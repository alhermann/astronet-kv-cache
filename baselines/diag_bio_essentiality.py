"""Bio-essentiality ablation: does the Ca²⁺-EMA dynamics matter?

Tests inference-time state-update variants on trained S2 (no retraining).
The trained queries/projections still work; we only change how g(t) is updated:

  1. default       — EMA (α≈0.39 learned, current method): g(t+1) = (1-α)g(t) + α s(t)
  2. alpha_1       — no integration, last-window only: g(t+1) = s(t)
  3. alpha_0       — no updates, state stays zero
  4. mean          — uniform mean across windows: g(t+1) = mean(s(0..t))
  5. last_only     — same as alpha_1 but explicit (sanity)

If `default` >> alternatives, the EMA dynamics carry weight. If default ≈ alpha_1,
the bio claim collapses: any post-hoc summary works.
"""
...
def process_with_state_variant(model, tokenizer, windows, question, k, variant, device, astro, sense_layer):
    ...
    for window in windows:
        ...
        with torch.no_grad():
            ...
            sensed_history.append(sensed.detach().clone())

    # Apply state-update variant
    if variant == 'default':
        for s in sensed_history:
            astro.update_state(s)
    elif variant == 'alpha_1':
        astro.g = sensed_history[-1].detach()
    elif variant == 'alpha_0':
        pass  # leave g at zero
    elif variant == 'mean':
        astro.g = torch.stack(sensed_history).mean(dim=0).detach()
    ...
    # Cross-window mult scoring (S1, unchanged)
...
def main():
    ...
    print(f"[bio-essentiality] {args.model_path}", flush=True)
    ...
    if args.multi_gpu:
        ...
        model = AutoModelForCausalLM.from_pretrained(args.model_path, quantization_config=bnb, device_map={'':'cuda:0'}, torch_dtype=torch.float16)
    model.eval()
    ...
    variants = ['default', 'alpha_1', 'alpha_0', 'mean']
    ...
    for trial in range(args.n_trials):
        for di, depth in enumerate(depths):
            ...
            for v in variants:
                pred = process_with_state_variant(model, tokenizer, windows, question, args.k, v, device, astro, sense_layer)
                ok = int(answer.lower() in pred.lower())
                results[v].append(ok)
                if trial < 2:
                    print(f"  trial={trial} d={depth} {v}: ok={ok}", flush=True)

    print(f"\n=== AGGREGATE bio-essentiality {args.save_tag} (n={args.n_windows}) ===", flush=True)
    for v in variants:
        acc = sum(results[v]) / len(results[v])
        print(f"  {v}: {acc:.3f} ({sum(results[v])}/{len(results[v])})", flush=True)

    with open(f'logs/results/diag_bio_essentiality_{args.save_tag}.json', 'w') as f:
        json.dump(results, f, indent=2)
    print(f"Saved to logs/results/diag_bio_essentiality_{args.save_tag}.json", flush=True)
