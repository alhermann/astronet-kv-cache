...
For additional cache compression we use the classical MMSE-optimal
Lloyd--Max scalar quantiser\cite{lloyd1982least,max1960quantizing},
which selects codebook levels that minimise expected squared
reconstruction error against a given source distribution. We
approached this design via TurboQuant\cite{zandieh2026turboquant}, a
recent two-stage rotation-based scheme that uses Lloyd--Max as its
inner step. TurboQuant has two stages: an MSE-optimal stage that
applies a random orthogonal rotation followed by per-coordinate
Lloyd--Max scalar quantisation, and a residual-correction stage that
uses quantised Johnson--Lindenstrauss projections to recover unbiased
inner-product estimates. The residual-correction stage requires
bypassing the attention multiply with a JL-based estimator, which is
incompatible with frozen-attention kernels; the rotation in the MSE
stage is, as we show next, incompatible with rotary position
encodings. We therefore retain only Lloyd--Max itself and bring it
into a RoPE-compatible form, with no rotation and no JL residual.
...
all channels scrambles this pairing. In our hands, TurboQuant at 4
bits reduces accuracy on Qwen 7B to zero on the multi-window SQuAD
task. The same failure mode is structurally expected on every
RoPE-using backbone we evaluate.

Our variant removes the rotation entirely. We instead normalise each
attention head's key and value coordinates independently to zero mean
and unit variance using head-level running statistics, apply the
classical Lloyd--Max scalar codebook tuned to the standard normal
distribution, and denormalise. The resulting scheme is per-(layer,
head) scalar Lloyd--Max --- a classical method --- with the per-head
Gaussianisation taking the role that rotation plays in TurboQuant. Our
contribution in this section is the diagnosis of the RoPE
incompatibility and the demonstration that the simplest fix (drop the
rotation, normalise per head) preserves quantisation accuracy on
Qwen, Llama and Mistral backbones; the codebook itself is not new.

We use 8-bit keys and 4-bit values (K8V4): keys participate in inner
products $\mathbf{q}\cdot\mathbf{k}$ that are sensitive to coordinate
noise, while values enter only through attention-weighted averages
that tolerate coarser representation. Per token retained, the K8V4
encoding uses $1.5\times d_h$ bytes versus $4\times d_h$ for FP16, a
$\sim$2.6$\times$ reduction; combined with the $10\times$ selection
compression for a 20-window context relative to the full cache, the
total is $\sim$26$\times$ at FP16 and $\sim$68$\times$ with K8V4
(Table~\ref{tab:compression}). The per-head normalisation step is
...
applying max-pool smoothing (kernel size 7) and a recent-token
reservation before selection, with selection performed per
(layer, head). StreamingLLM\cite{xiao2024streamingllm} retains a
small number of initial attention sinks together with a sliding
window of the most recent tokens. PyramidKV\cite{cai2024pyramidkv}
allocates a layer-adaptive budget that decreases with depth,
motivated by an observed concentration of useful attention in
shallow layers. KIVI\cite{liu2024kivi} compresses the retained cache
through asymmetric integer quantisation, with per-channel
quantisation for keys and per-token quantisation for values; we
evaluate KIVI at 4 bits (K4V4) as the operating point matched in
cache-byte cost to our K8V4 stack.
...
\textcolor{red}{\textbf{[METHODOLOGICAL CORRECTION — IN PROGRESS]}} An
earlier version of this paper used hand-rolled re-implementations of
H${}_2$O, SnapKV and StreamingLLM with a global-pooling
modification (scoring each token by averaging across attention heads
before ranking) and applied them inside AstroNet's window-streaming
protocol. On audit those re-implementations were found NOT faithful
to the published methods: SnapKV uses per-(layer, head) selection
with a max-pool kernel-7 over an end-of-prompt observation window,
H${}_2$O accumulates attention across decoding steps, and both
operate over a single-prompt prefill. The original ``per-head
collapse to $\sim 8.5\%$'' justification documented in this section
reflected the limitations of our re-implementation, not of the
published algorithms. We are re-running all baseline comparisons
with the upstream KVCache-Factory monkey-patches
\cite{cai2024pyramidkv} which provide faithful per-(layer, head)
SnapKV / H${}_2$O / PyramidKV. Two operating regimes are reported in
the revised tables: single-prompt (the regime the published methods
target, used as oracle upper bound) and streaming-protocol adaptation
(matched to AstroNet's protocol). Numbers from the previous
hand-rolled baseline rows have been withdrawn from
Tables~\ref{tab:longbench}, \ref{tab:needle}, \ref{tab:ruler}, and
\ref{tab:latency} pending the upstream re-runs.
